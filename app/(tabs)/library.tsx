    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#121212' }} edges={['top']}>
      ...
      <View className="flex-row items-center justify-between px-4 py-4">
        <View className="flex-row items-center gap-3">
          <View className="h-8 w-8 items-center justify-center rounded-full bg-[#E1118C]">
            <Text className="text-xs font-bold text-black">J</Text>
          </View>
          <Text className="text-2xl font-bold tracking-tight text-white">Your Library</Text>
        </View>
        ...
        <ScrollView
          ...
          {CATEGORIES.map((cat, i) => (
            <Pressable key={i} className="rounded-full border border-white/20 px-4 py-2">
              <Text className="text-xs font-semibold text-white">{cat}</Text>
            </Pressable>
          ))}
        </ScrollView>
      </View>
      ...
      <View className="flex-row items-center justify-between px-4 py-2">
        <Pressable className="flex-row items-center gap-2">
          ...
          <Text className="text-xs font-semibold text-white">Recent</Text>
        </Pressable>
        <Pressable>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <Pressable className="flex-row items-center gap-4 px-4 py-3">
          <View className="h-16 w-16 items-center justify-center rounded-sm bg-[#2a2a2a]">
            <Plus size={32} color="#b3b3b3" />
          </View>
          <Text className="text-base font-bold text-white">Add playlists</Text>
        </Pressable>
            ...
            <Pressable className="flex-row items-center gap-4 px-4 py-3">
              ...
              <View className="flex-1">
                ...
                <View className="flex-row items-center gap-1">
                  {item.isPinned && <AudioLines size={14} color="#22c55e" />}
